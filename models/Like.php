...
class Like extends Model
{
    ...
    {
        $id = 'like_' . uniqid();
        $sql = "INSERT INTO {$this->table} (_id, client_id, product_id, created_at, updated_at) 
                VALUES (:id, :client_id, :product_id, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)";
    ...
    {
        $sql = "DELETE FROM {$this->table} WHERE client_id = :client_id AND product_id = :product_id";
        $stmt = $this->db->prepare($sql);
        return $stmt->execute([
            ':client_id' => $clientId,
            ':product_id' => $productId
    ...
    {
        $sql = "SELECT * FROM {$this->table} WHERE client_id = :client_id AND product_id = :product_id LIMIT 1";
        $stmt = $this->db->prepare($sql);
        $stmt->execute([
            ':client_id' => $clientId,
            ':product_id' => $productId
    ...
    {
        $sql = "SELECT product_id FROM {$this->table} WHERE client_id = :client_id ORDER BY created_at DESC";
        $stmt = $this->db->prepare($sql);
